...
Aus der Pixelverteilung wird nun ein Ausgleich berechnet welche den kumulativen Pixelwert durch die Anzahl aller Pixel teilt und das Ergebnis wiederum durch die höchste Graustufe. Das Ergebnis wird nun gerundet und der passenden Graustufe zugeordnet. Die Vorgehensweise mit Farbbildern funktioniert gleich, nur das die R, G und B Kanäle verwendet werden. Häufig wird für diese Verfahren, ein Farbraum gewählt, welcher einen Kanal für die Luminanz besitzen.
  \begin{table}
  [h]
...
Auch in dieser Herleitung bildet die Gesamtzahl an Graustufen (Normalerweise 256). Im weiteren wird versucht, jeden $r$-Wert in $X$ auf den $z$-Wert abzubilden, welche die gleiche Wahrscheinlichkeit haben. Das heißt:

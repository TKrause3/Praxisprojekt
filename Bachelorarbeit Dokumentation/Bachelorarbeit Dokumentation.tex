...
\end{enumerate}
Um die Auswirkungne der Normalisierungsverfahren vergleichen zu können werden verschiedene Verfahren auf die selben trainingsdaten angewendet. Durch die Normalisierungsverfahren soll die Farbvarianz verringert werden ???? Anwendung der Normaliesirung auf Trainigsdaten oder Testdaten oder beiden ??? .
...
Als Aktivierungsfunktion wird häufig die Sigmoid-Funktion ??? was is das kurz beschreiben oder weglassen ??? verwendet, ist aber auch mit anderen linearen und nicht linearen Funktionen möglich. Zusätzlich wird dem Ergebnis der Funktion der \textit{Bias} addiert.\\
\begin{eqnarray} g(( \sum_{i=0}^n x_{i} *w_{i}) +b \end{eqnarray}\\
Der \textit{Bias} ist für das Verringern von Konvergenz-Problemen, welche bei der Approximation der Funktion auftreten können. Der Bias hat ein Wert von 1 und hat ein Gewicht, welches unabhängig der Eingabewerte ist ??? nicht klar beschrieben was der Bias wirklich is, b = 1*g  ??? .\\\\
...
dabei stellt\\
	...
	$f$ den Sollwert dar ??? Fehlt hier nicht auch das summen zeichen, entweder bei beiden weglassen, würde ich machen oder bei beiden hinschreiben auserdem den index "n" mit an die funktionen schreiben ???\\\\
...
Es existieren eine menge von Filtern $n$ welche eine Höhe von $f_{h}$, einer Breite von $f_{w}$ und einer tiefe von $f_{d}$ ??? digitales bild hat nur 2 ebenen also kein tiefe $f_{d}$ und $e_{d}$ kommen in deinen Formeln auch nicht mehr vor oder? ??? haben. Dieser Filter wird über das Eingabebild mit der Höhe $e_{h}$, der breite $e_{w}$ und der Tiefe von $e_{d}$ gelegt und in ein Pixel schritten bewegt. Die Ergebnisse stellen eine Liste von Merkmalen da welche auf der Höhe $m_{h}$, breite $m_{w}$ und tiefe $m_{d}$ liegen. Formell ausgedrückt:\\
\begin{eqnarray} 
m_{h}&=&(e_{h} - f_{h})+1\\
m_{w}&=&(e_{w} - f_{w})+1
\end{eqnarray}\\
...
p_{w}&=&f_{w} - 1
\end{eqnarray}
Die Faltungsschicht hat eigene Gewichte, welche beim Vorgang mit denen des Bildes zusammengerechnet wird \cite[331ff.]{goodfellow2016deep}. Für die gleiche Schicht wird derselbe Filter verwendet. Die Weise der Berechnung wird in der Abbildung ??? Verweis auf die Abbildung ???  beschrieben. ??? Bild an der falschen stelle im PDF???\\
\begin{figure}
	[h]
	\centering
...
nachdem das Eingabebild durch alle Faltungs- und Pooling Schichten verarbeitet wurde, werden die Daten an den Fully connected Layer übergeben \cite[14]{sermanet2012convolutional}. Dieser verwendet, wie bei einem normalen KNN, Perzeproten um die Daten zu verarbeiten. Die vorherigen Schichten wurden durchlaufen, damit diese rechenintensive Schicht nicht zu viel Informationen verarbeiten muss. 
  \subsubsection{Entstehung von Trainingsdaten}\label{s.trainingsdaten} ??? Dieser Abschnitt ist sehr gemischt mit der konkreten umsetzung ???
Damit ein künstliches neuronales Netz Objekte erkennen und zuordnen kann, benötigt es zunächst eine Reihe von Beispieldaten. Anhand dieser Beispieldaten lernt das neuronale Netz, welche Eigenschaften das zu erlernende Objekt hat und wie es aufgebaut ist. Für diesen Vorgang werden üblicherweise mehrere Hunderttausend Daten benötigt, um ein akzeptables Ergebnis zu erzielen. Da für ein solches Training nicht die benötigten Ressourcen zur Verfügung stehen, sollte eine Möglichkeit gefunden werden, wie das Training auch mit weniger Trainingsdaten durchgeführt werden kann ??? Dieser Satz gehört zu durchführung nicht in den Theorewtischen teil???. 
...
Klassenname & AP(N) & AP(GW) & AP(HA) & AP(HS)\\
\hline
Orange & 1.000 & 1.000 & 1.000 & 0.000\\
Nektarine & 1.000 & 1.000 & 1.000 & 0.000\\
Banane & 1.000 & 1.000 & 1.000 & 0.000\\
Mango & 1.000 & 1.000 & 1.000 & 0.000\\
Apfel & 1.000 & 1.000 & 0.999 & 0.000\\
Birne & 1.000 & 1.000 & 1.000 & 0.000\\
\hline
mAP & 1.000 & 1.000 & 1.000 & 0.000\\
\hline
\end{tabular}
\end{table}\\
Die Auswertung der normalisierten Trainingsdaten hat ergeben, das wesentlich weniger farb-Anomalien auftreten. Dabei ist aufgefallen das gerade bei den Histogramm Normalisierungen mit einheitlichen Hintergrund deutliche Verbesserungen erzielt wurden. Zu große Veränderungen treten nur dann auf, wenn zu viele Objekte auf dem Bild enthalten sind, da diese die Umgebung zu stark abändern. Die Histogramm Spezifikation geht mit diesem Problem aber noch am besten um, weil dieser immer ein Referenz-Histogramm hat, um es anzupassen.        
  \newpage
\section{Diskussion}
